...
## Using screenmed to screen abstracts for a systematic review or a meta-analysis 
...
Initially we applied a search in Pubmed and saved the result in a csv file. We also knew 4 publications that we were quite sure that belong to the study.


```{r}
initial_search <- read.csv("csv-randomized-set.csv")
initialPMID<-initial_search$ο.ΏPMID
knownPMID<-c("25641242","18822428","8276025","16452355")
```
...
```{r}
b_cluster<-screenmed(secondPMID,knownPMID,0.99,2)
...
So we ended up with 153+63=216 possible abstracts to check out manually. Not bad. If we can this that we have started from 581. Our colleagues did the job for us. The did the whole check manually and found that the rest are:
```{r}
rest_relevant_pubs<-c("17329276","8835086","23904065","11023168","10356137","9175947","8215566","15930210","8346957","8021760")
```
...
```
That's correct. Everything is included. Not even needed to check the ones that RISmed could not provide us with abstracts (RISmed is the program that runs behind). The program succeeded to achieve more or less reduction of the manual abstract check to 153/581= 0.26 (26 %) Not bad at all. Some advice to end up. The process here was a small test. There are systematic reviews or meta-analyses that include tens of thousands abstracts. In that case, would be a good idea, to split the initial PMID vector to chunks of 1000 abstract PMIDS and to repeat the process as shown above. One could end up saving a lot of time using the program.
