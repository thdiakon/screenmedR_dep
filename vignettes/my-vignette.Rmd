...
screenmedR is a package for automatizing the screening of publications in order to minimize the manual work needed to extract the proper publications for your meta-analysis project. It uses unsupervised machine learning algorithms to divide in groups all abstracts in terms of their cosine similarity with a small relevant set  of 4 or 5 abstracts which you think they belong to your meta-analysis. The group with the highest cosine similarity can be chosen (in case the difference with the others is big enough) to narrow your search and end up with a much smaller number of abstracts needed for checking. 
It comes also with extra functions that use mesh terms (Descriptors & Qualifiers) so as to narrow down your search even more. It uses RISmed [@RISmed] and rentrez [@rentrez] packages to extract the information that needed and tm[@tm] package for the NLP.
...
This is a simple example of implementation of the code for the screenmedR package.
...
## Using screenmed to screen abstracts for a meta-analysis

Here, an example is provided, from a meta-analysis publication [@] in order to    understand how the program works. One can find more information on  how the program works by reading the above publication.

As a first step we apply a search in Pubmed and save the result in a csv file. We also input 5 publications that we were quite sure that belong to the study.
...
The third number in function screenmed is useful for the run and sometimes is quite useful for the clustering. It is the number applied to the removesparseterms() function of tm package[@tm]. 
It actually removes terms, words that occur very rare (in this case less than 0,5% of the document term matrix). One can play with it between (0,99, 1) to have a better clustering. (Bigger difference in Cosine similarity).
...
The abstractsofgroup() function filters the PMIDS of the abstracts that belong to the first group (the one with the bigger cosine similarity).
The "clustering" term, gives the group each abstract belong to while the second number in the function, the group that we want the PMIDS of the abstracts to extract. 

What is left is a second clustering, using the rules of the first: 
...
So we ended up with 118+63=181 possible abstracts to check out manually. Not bad. We have started from 581. Our colleagues on the same time by doing the whole check manually found that the rest are:
```{r}
rest_relevant_pubs<-c("17329276","8835086","23904065","11023168","10356137","9175947"
                      ,"8215566","15930210","8346957","33627329")
```

Can the program filter correctly the appropriate publications? We can check it out. The cosine similarity result shows that the second group should hold our rest_relevant_pubs:
...
### The mesh_clean() and mesh_clean_bq() functions
...
We provide with our initial search group (first term) and the small group of PMIDs of the publications (second term) and ask for at least 11 Descriptors in common and 2 Qualifiers.  
